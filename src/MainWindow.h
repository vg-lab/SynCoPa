...
public:
    ...
    SynCoPaWebAPI& getWebApi();

    const SynCoPaWebAPI& getWebApi() const;

    const std::shared_ptr<SynCoPaWebSocket>& getWebSocket() const;
...
protected:
    ...
    void loadPostsynapticList(std::vector<unsigned int> gid);
    ...
    std::shared_ptr<SynCoPaWebSocket> _web_socket;
